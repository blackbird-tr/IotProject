...
//ESP32Time rtc;
ESP32Time rtc(3600);  
const long gmtOffset_sec = 3600;        // Adjust this value based on your timezone
const int daylightOffset_sec = 3600;    // Adjust this value based on your daylight saving time
const char* ntpServer = "pool.ntp.org";// offset in seconds GMT+1
/* --- Public Functions --- */ 
void azure_pnp_init() {
 ...
 pinMode(PIR_PIN, INPUT);
  ...
  // Lamba pin modu çıkış olarak ayarlanıyor
...
static void simulated_get_pressure_altitude(float* quality)
{
    
  *quality = analogRead(MQ135_AOUT_PIN); 
}

static float simulated_get_motion() { 
    ...
    size_t* payload_buffer_length)
{
  ...
  motion=simulated_get_motion();
  simulated_get_pressure_altitude(&quality);  
